...
@section('content')
    <h2 class="mb-3">Daftar Pengaduan Pelayanan</h2>

    <!-- Filter -->
    <form method="GET" action="{{ route('admin.pelayanan') }}" class="d-flex align-items-center mb-3">
        ...
        <a href="{{ route('admin.pengaduan_pelayanan.pdf', ['filter' => request('filter')]) }}" class="btn btn-success ms-3">
            Download PDF
        </a>
    </form>
    ...
    <form action="{{ route('admin.multi_delete') }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data yang dipilih?')">
        ...
        <table class="table table-bordered align-middle">
            <thead class="table-light">
                <tr>
                    <th><input type="checkbox" id="select-all-pelayanan"></th>
                    <th>Nama</th>
                    <th>NIP</th>
                    <th>Penjelasan</th>
                    <th>File</th>
    ...
    <script>
        document.getElementById('select-all-pelayanan').addEventListener('change', function() {
